// GPU Test: DrawUnderline - 下划线渲染验证
package main
...
// UnderlineTestWidget 自定义控件，用于渲染下划线测试
type UnderlineTestWidget struct {
	widget.BaseWidget
}

func NewUnderlineTestWidget() *UnderlineTestWidget {
	w := &UnderlineTestWidget{
...
func main() {
	...
	app := ui.NewApplication("GPU Test: DrawUnderline", 1600, 900)
...
func setupUI(engine *ui.Engine) {
	...
	if outDir == "" {
		outDir = "lcl/gpui/test_output/gpu_tests/underline"
	}
	...
	// 标题
	title := widget.NewText("GPU Test: DrawUnderline (下划线渲染)")
	title.SetPos(40, 20)
	...
	// 测试标签
	label1 := widget.NewText("1. 文本下划线 (Hello World)")
	label1.SetPos(40, 80)
	...
	label2 := widget.NewText("2. 不同厚度下划线 (thickness=1,2,3,4)")
	...
	label3 := widget.NewText("3. 不同颜色下划线 (黑/红/蓝/绿)")
	...
	label4 := widget.NewText("4. 不同长度下划线 (50,100,200,400)")
	...
	// 添加自定义渲染控件
	testWidget := NewUnderlineTestWidget()
...
1. 文本下划线: 下划线位于文本底部，与文本对齐，无偏移
2. 不同厚度: 4条下划线，厚度分别为1,2,3,4像素，厚度差异明显可见
3. 不同颜色: 4条下划线，颜色分别为黑/红/蓝/绿，颜色清晰可辨，无混色
4. 不同长度: 4条下划线，长度分别为50,100,200,400像素，长度差异明显可见
验证标准: 所有下划线为水平线条，位于文本底部，厚度/颜色/长度符合预期`)
	...
	go func() {
		...
		lcl.RunOnMainThreadAsync(func(id uint32) {
			imgPath := filepath.Join(outDir, "gpu_underline.png")
			engine.Renderer().SavePNG(imgPath)
			fmt.Printf("✓ 图片已保存: %s\n", imgPath)
		})
	}()

	fmt.Println("✓ GPU测试程序已启动: DrawUnderline")
}
